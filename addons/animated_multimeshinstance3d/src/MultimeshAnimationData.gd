...
func set_values(input_start_frame : int, input_length) -> Resource:
	start_frame = input_start_frame
	length = input_length
	return self

## Encodes animation data into a custom_data buffer format (Forward+ renderer only).
static func encode_animation_forward_plus(data) -> float:
	if data == null:
		return 0.0
...
static func dencode_animation_forward_plus(buffer_channel : float) -> Resource:
	var values = decode_two_integers_forward_plus(buffer_channel)

	return load("res://addons/animated_multimeshinstance3d/src/MultimeshAnimationData.gd").new().set_values(
		values[0],
		values[1]
	)
