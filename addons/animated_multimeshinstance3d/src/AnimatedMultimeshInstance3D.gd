...
const ANIM_DATA_SCRIPT_PATH = "res://addons/animated_multimeshinstance3d/src/MultimeshAnimationData.gd"
...
@export var animation_list : Dictionary[String, Resource] = {}
...
func _update_new_animation() -> void:
	...
	animation_list[current_animation_name] = \
		load(ANIM_DATA_SCRIPT_PATH).new().set_values(
...
func get_animation_data(animation_name: StringName = &"") -> Resource:
...
func play(
	...
	var animation = animation_list[animation_name]
...
func get_animation(instance : int) -> Resource:
	if _is_renderer_forward_plus:
		var alpha_channel_data : PackedInt64Array = load(ANIM_DATA_SCRIPT_PATH).decode_two_integers_forward_plus(
		...
		if alpha_channel_data[1] != 0:  ## Blend out time is not zero, meaning the animation blends back to original animation
			return load(ANIM_DATA_SCRIPT_PATH).dencode_animation_forward_plus(
...
func get_current_animation_name(instance : int) -> String:
	...
	var current_animation = get_animation(instance)
...
func combine_custom_buffer(
	main_animation,
	blended_animation = main_animation,
	blend_duration : float = 0.0,
	blend_out_time : float = 0.0
) -> Color:
	if _is_renderer_forward_plus:
		...
		return Color(
			load(ANIM_DATA_SCRIPT_PATH).encode_animation_forward_plus(main_animation),
			load(ANIM_DATA_SCRIPT_PATH).encode_animation_forward_plus(blended_animation),
			blend_timestamp,
			load(ANIM_DATA_SCRIPT_PATH).encode_two_integers_forward_plus(
